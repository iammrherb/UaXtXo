 ...
 * Coordinates dashboard loading, wizard integration, and all components
 */
...
async function initializeApplication() {
    ...
    try {
        ...
        // Phase 5: Fix vendor images
        await fixVendorImages();
...
async function fixVendorImages() {
    // Fix all vendor images to use correct paths
    document.querySelectorAll('img[src*="vendor"]').forEach(img => {
        img.onerror = function() {
            // Don't create placeholders, just try alternate paths
            const originalSrc = this.src;
            const filename = originalSrc.split('/').pop();
            
            // Try different paths if image not found
            if (originalSrc.includes('no-nac-icon.svg')) {
                this.src = 'img/vendors/no-nac.png';
            } else if (originalSrc.includes('placeholder-logo.png')) {
                this.src = 'img/portnox-logo.png'; // Use main logo as fallback
            } else {
                // For vendor logos, try without the '-logo' suffix
                const vendorName = filename.replace('-logo', '').replace('.png', '');
                this.src = `img/vendors/${vendorName}.png`;
            }
