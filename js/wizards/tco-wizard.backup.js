...
(function() {
    // Override the calculateAndClose function to integrate with dashboard
    const originalCalculateAndClose = TCOWizard.calculateAndClose;
    
    TCOWizard.calculateAndClose = function() {
        const config = TCOWizard.collectWizardData();
        
        // Pass to calculator
        if (typeof Calculator !== 'undefined') {
            Calculator.updateState(config);
            const calculations = Calculator.calculateTCO();
            
            // Create event with wizard data for dashboard
            const wizardCompleteEvent = new CustomEvent('tco-wizard-complete', {
                detail: {
                    config: config,
                    calculations: calculations,
                    vendors: TCOWizard.getSelectedVendorDetails()
                }
            });
            document.dispatchEvent(wizardCompleteEvent);
        }
        
        // Call original function
        originalCalculateAndClose.call(this);
    };
    
    // Add method to get vendor details
    TCOWizard.getSelectedVendorDetails = function() {
        const selectedVendors = TCOWizard.getSelectedVendors();
        return selectedVendors.map(vendorId => ({
            id: vendorId,
            name: TCOWizard.vendorDetails[vendorId].name,
            type: TCOWizard.vendorDetails[vendorId].type,
            logo: `${vendorId}-logo.png`
        }));
    };
})();

// Fix vendor logo display in wizard
(function() {
    // Override vendor card creation to use proper logo paths
    const originalCreateVendorCards = TCOWizard.createVendorSelectionStep;
    
    TCOWizard.createVendorSelectionStep = function() {
        const html = originalCreateVendorCards.call(this);
        
        // Initialize logo loader after DOM update
        setTimeout(() => {
