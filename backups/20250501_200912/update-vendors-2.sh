...
# Simplified NAC Solution TCO Calculator Update Script
# This script updates the NAC Solution TCO Calculator with all necessary fixes
...
download_file() {
    ...
    if [ ! -f "$file_path" ]; then
        ...
        curl -s -L -o "$file_path" "$url" || echo "Failed to download $url"
    else
...
if [ -f "${PROJECT_DIR}/index.html" ]; then
    ...
    # Insert feature comparison and ROI charts before the Why Portnox Cloud section
    content=$(cat "${TEMP_DIR}/index.html")
    charts=$(cat "${TEMP_DIR}/chart-canvas.html")
    content=$(echo "$content" | sed "/<div class=\"comparison-highlight-card\">/i\\$charts")
    
    # Insert implementation chart in the implementation tab
    impl_chart=$(cat "${TEMP_DIR}/implementation-chart.html")
    content=$(echo "$content" | sed "/<div id=\"implementation-tab\" class=\"tab-pane\"/a\\        $impl_chart")
    
    # Save the modified content
    echo "$content" > "${PROJECT_DIR}/index.html"
...
if grep -q "advanced-options-panel" "${PROJECT_DIR}/index.html"; then
    ...
    # Insert custom costs section after advanced options panel
    custom_costs=$(cat "${TEMP_DIR}/custom-costs.html")
    content=$(cat "${TEMP_DIR}/index.html")
    content=$(echo "$content" | sed "/<div id=\"advanced-options-panel\" class=\"advanced-options-panel hidden\"/a\\                        <!-- Custom Cost Configuration -->\\$custom_costs")
...
cat > "${PROJECT_DIR}/sensitivity.html" << 'EOF'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
cat > "${PROJECT_DIR}/css/sensitivity.css" << 'EOF'
...
cat > "${PROJECT_DIR}/js/components/sensitivity-analyzer.js" << 'EOF'
/**
...
cat > "${PROJECT_DIR}/js/sensitivity.js" << 'EOF'
/**
...
if [ -f "${PROJECT_DIR}/index.html" ] && ! grep -q "jspdf.umd.min.js" "${PROJECT_DIR}/index.html"; then
    ...
    # Add jsPDF after Chart.js
    content=$(cat "${TEMP_DIR}/index.html")
    content=$(echo "$content" | sed '/<script src="libs\/charts\/chart.min.js"><\/script>/a \
    <!-- jsPDF for PDF export -->\
    <script src="libs/jspdf/jspdf.umd.min.js"></script>\
    <script src="libs/jspdf/jspdf.plugin.autotable.min.js"></script>')
...
if [ -f "${PROJECT_DIR}/js/components/ui-controller.js" ]; then
    ...
    if grep -q "exportToCSV()" "${PROJECT_DIR}/js/components/ui-controller.js"; then
        ...
        line_num=$(grep -n "exportToCSV()" "${PROJECT_DIR}/js/components/ui-controller.js" | head -1 | cut -d: -f1)
        if [ -n "$line_num" ]; then
            # Find the closing brace after exportToCSV
            awk_script="NR >= $line_num {if (/^  }/) {print NR; exit}}"
            end_line=$(awk "$awk_script" "${PROJECT_DIR}/js/components/ui-controller.js")
            ...
            if [ -n "$end_line" ]; then
                ...
                awk -v line="$end_line" -v func="$export_to_pdf" 'NR==line{print; print "\n" func} NR!=line{print}' "${TEMP_DIR}/ui-controller.js" > "${PROJECT_DIR}/js/components/ui-controller.js"
                
                echo "Added exportToPDF function to ui-controller.js"
            else
                echo "Warning: Could not find the end of exportToCSV function"
            fi
        else
            echo "Warning: Could not find exportToCSV function"
        fi
    else
...
if [ -f "${PROJECT_DIR}/js/components/calculator.js" ]; then
    ...
    # Replace the existing calculateVendorTCO function with our updated version
    calculate_vendor_tco=$(cat "${TEMP_DIR}/calculate-vendor-tco.js")
    
    awk '
    /calculateVendorTCO\(vendor, currentVendor, orgSize, deviceCount, yearsToProject\)/ {
        in_function = 1
        print "'"$calculate_vendor_tco"'"
        next
    }
    ...
    }' "${TEMP_DIR}/calculator.js" > "${PROJECT_DIR}/js/components/calculator.js"
