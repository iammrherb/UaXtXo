...
class RiskSecurityModule {
    ...
    renderSecurityMaturityRadar(results) {
        const container = document.getElementById('security-maturity-radar');
        if (!container) {
            console.error('Security maturity radar container not found');
            return;
        }
    ...
    renderZeroTrustGauge(results) {
        const container = document.getElementById('zero-trust-gauge');
        if (!container) {
            console.error('Zero trust gauge container not found');
            return;
        }
    ...
    renderAttackSurfaceChart(results) {
        ...
        if (!container) {
            console.error('Attack surface chart container not found');
            return;
        }
    ...
    renderSecurityControlsHeatmap(results) {
        const container = document.getElementById('security-controls-heatmap');
        if (!container) {
            console.error('Security controls heatmap container not found');
            return;
        }
    ...
    renderVulnerabilityTimeline(results) {
        const container = document.getElementById('vulnerability-timeline');
        if (!container) {
            console.error('Vulnerability timeline container not found');
            return;
        }
    ...
    renderBreachCostProjection(results) {
        const container = document.getElementById('breach-cost-projection');
        if (!container) {
            console.error('Breach cost projection container not found');
            return;
        }
        ...
        Highcharts.chart(container, {
            ...
            tooltip: {
                ...
                formatter: function() {
                    return '<b>' + this.x + '</b><br/>' +
                           this.series.name + ': $' + Math.round(this.y / 1000) + 'K';
                }
            },
            legend: {
                itemStyle: { color: '#CBD5E1' },
    ...
    renderRiskProbabilityMatrix(results) {
        const container = document.getElementById('risk-probability-matrix');
        if (!container) {
            console.error('Risk probability matrix container not found');
            return;
        }
    ...
    renderExecutiveRiskSummary(results) {
        const metrics = this.calculateRiskMetrics(results);
        const breachCostThousands = Math.round(this.platform.config.breachCost * 0.15 / 1000);
        const investmentThousands = Math.round((results.portnox?.year1?.tco?.total || 50000) / 1000);
        const netReductionThousands = Math.round(metrics.breachSavings * 0.8);
                    ...
                    <div class="summary-section">
                        ...
                        <div class="impact-metrics">
                            <div class="metric">
                                ...
                                <span class="value">$${breachCostThousands}K</span>
                            </div>
                            <div class="metric">
                                <span class="label">Mitigation Investment</span>
                                <span class="value">$${investmentThousands}K</span>
                            </div>
                            <div class="metric">
                                <span class="label">Net Risk Reduction</span>
                                <span class="value positive">$${netReductionThousands}K</span>
                            </div>
                        </div>
                    </div>
