...
echo "=== Installing NAC Architecture Designer Pro Fixes ==="

# Check for index.html
if [ -f "index.html" ]; then
    echo "Found index.html, backing up..."
    cp index.html index.html.fixes-backup
    
    # Check if fixes already installed
    if grep -q "nac-fixes.js" index.html; then
        echo "Fixes already installed in index.html"
    else
        # Add fix loader to head section
        echo "Adding fix loader to index.html..."
        sed -i '/<\/head>/i <script src="js/fixes/nac-fixes.js"></script>' index.html
        echo "Fix installation in index.html complete"
    fi
else
    echo "WARNING: index.html not found, cannot automatically install fixes"
    echo "You need to manually add this line to your HTML file before the closing </head> tag:"
    echo '<script src="js/fixes/nac-fixes.js"></script>'
fi

# Create a test script to verify fixes
cat > test-fixes.html << 'EOF2'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
echo "To test the fixes, open test-fixes.html in your browser"
echo ""
echo "If you need to manually add the fixes to other HTML files, add this line:"
echo '<script src="js/fixes/nac-fixes.js"></script>'
