...
echo "Starting NAC TCO Calculator Chart Restoration Script"

# Fix chart-init-safe.js (currently disabled)
cat > js/charts/chart-init-safe.js << 'EOF'
/**
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.Chart) {
        ...
        // Register custom plugin to handle responsiveness
        ...
        Chart.register(responsivePlugin);
...
# Fix syntax errors in enhanced-chart-builder.js
echo "Fixing enhanced-chart-builder.js..."
sed -i'.bak' -e "s/return ' + /return '$ + /g" js/charts/enhanced-chart-builder.js
sed -i'.bak' -e "s/label += ' + /label += '$ + /g" js/charts/enhanced-chart-builder.js
sed -i'.bak' -e "s/return 'Total Cost:  + /return 'Total Cost: $ + /g" js/charts/enhanced-chart-builder.js
sed -i'.bak' -e "s/return `\${context.dataset.label}: \${context.raw.toFixed(1)}%`;/return context.dataset.label + ': ' + context.raw.toFixed(1) + '%';/g" js/charts/enhanced-chart-builder.js
sed -i'.bak' -e "s/return `\${context.dataset.label}: \${context.raw}\/10`;/return context.dataset.label + ': ' + context.raw + '\/10';/g" js/charts/enhanced-chart-builder.js

# Fix chart-builder.js to restore functionality
echo "Restoring chart-builder.js functionality..."
...
# Fix integrated-sensitivity.js syntax errors
echo "Fixing integrated-sensitivity.js..."
sed -i'.bak' -e "s/return `\${context.dataset.label}: \${context.raw.toFixed(1)}%`;/return context.dataset.label + ': ' + context.raw.toFixed(1) + '%';/g" js/features/sensitivity-analysis/integrated-sensitivity.js
sed -i'.bak' -e "s/return `\${label}: \${value} \${unit}`;/return label + ': ' + value + ' ' + unit;/g" js/features/sensitivity-analysis/integrated-sensitivity.js
sed -i'.bak' -e "s/return `\${label}: \${window.formatCurrency ? window.formatCurrency(value) : '\$' + value.toLocaleString()}`;/return label + ': ' + (window.formatCurrency ? window.formatCurrency(value) : '$' + value.toLocaleString());/g" js/features/sensitivity-analysis/integrated-sensitivity.js

# Create a helper script to fix wizard functionality
echo "Creating wizard restoration script..."
cat > fix-wizard.js << 'EOF'
/**
...
# Create a patch fix script that can be run to apply all fixes
...
echo "Chart Restoration Script has been created"
echo "Run ./apply-fixes.sh to apply the fixes"
