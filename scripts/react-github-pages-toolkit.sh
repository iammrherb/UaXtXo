...
load_github_token() {
  if [ -f ".env.github" ]; then
    ...
    if [ ! -z "$GITHUB_TOKEN" ]; then
      ...
      # Configure git with token immediately
      if [ ! -z "$GITHUB_USERNAME" ]; then
        git remote set-url origin "https://${GITHUB_USERNAME}:${GITHUB_TOKEN}@github.com/${GIT_USERNAME}/${REPO_NAME}.git"
      else
      ...
      if git ls-remote origin > /dev/null 2>&1; then
        ...
        echo -e "${YELLOW}Stored token failed. Will prompt for new one.${NC}"
        configure_token
        return $?
      fi
    fi
  ...
  echo -e "${YELLOW}No working GitHub token found. Running configuration...${NC}"
  configure_token
  return $?
}

# Configure git with token if available - now handled in load_github_token
...
configure_token() {
  ...
  local repo_owner=$(echo $remote_url | sed -E "s|https://github.com/([^/]+)/.*|\1|")
  local repo_name=$(echo $remote_url | sed -E "s|https://github.com/[^/]+/([^.]+).*|\1|")
  
  echo -e "Repository: ${YELLOW}$repo_owner/$repo_name${NC}"
  
  # Prompt to confirm or modify repo details
  read -p "Is this correct? If not, enter the correct owner/name or press Enter to confirm: " repo_correction
  
  if [ ! -z "$repo_correction" ]; then
    # Parse the corrected info
    repo_owner=$(echo $repo_correction | cut -d "/" -f1)
    repo_name=$(echo $repo_correction | cut -d "/" -f2)
    echo -e "Updated to: ${GREEN}$repo_owner/$repo_name${NC}"
    
    # Update global variables
    GIT_USERNAME="$repo_owner"
    REPO_NAME="$repo_name"
  fi
  
  # Always prompt for token - dont use stored one if its failing
  echo -e "${YELLOW}Enter your GitHub personal access token:${NC}"
  read -s token
  
  if [ -z "$token" ]; then
    echo -e "${RED}No token provided. Authentication may fail.${NC}"
    return 1
  fi
  
  # For fine-grained tokens we need the username
  echo -e "${YELLOW}Enter your GitHub username (needed for fine-grained tokens):${NC}"
  read username
  
  # Save token securely
  echo "GITHUB_TOKEN=$token" > .env.github
  if [ ! -z "$username" ]; then
    echo "GITHUB_USERNAME=$username" >> .env.github
  fi
  ...
  if git ls-remote origin > /dev/null 2>&1; then
    ...
    echo -e " - Your token has the correct permissions (repo scope)"
    echo -e " - For fine-grained tokens, ensure it has access to this repository"
    echo -e " - Your username is correct"
    echo -e " - The repository owner and name are correct"
