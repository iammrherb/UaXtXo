...
import CostComparisonChart from "./enhanced-charts/cost-comparison-chart"
import ROITimelineChart from "./enhanced-charts/roi-timeline-chart"
import SecurityRadarChart from "./enhanced-charts/security-radar-chart"
...
import type { EnhancedCalculationResult } from "@/lib/services/calculation-service"
...
export default function TcoAnalyzerUltimate() {
  ...
  const [results, setResults] = useState<EnhancedCalculationResult[]>([])
  ...
  const loadSavedData = async () => {
    try {
      ...
      const saved = await CalculationService.loadCalculation(sessionId)
      if (saved) {
  ...
  const calculateResults = useCallback(async () => {
    ...
    try {
      const newResults = await CalculationService.compareVendors(selectedVendors, configuration)
      setResults(newResults)
      setLastCalculated(new Date())
      
      // Save to database and localStorage
      await CalculationService.saveCalculation(sessionId, configuration, selectedVendors, newResults)
