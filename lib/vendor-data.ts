...
export const vendorDatabase: Record<string, VendorInfo> = {
  portnox: {
    id: "portnox",
    name: "Portnox",
    description: "Cloud-native NAC solution with zero-trust security and rapid deployment",
    category: "enterprise",
    logo: "/portnox-logo.png",
    strengths: [
      "Fastest deployment in market (2-4 weeks)",
...
export function getVendorsByCategory(category: string): VendorInfo[] {
  return Object.values(vendorDatabase).filter((vendor) => vendor.category === category)
}

export function searchVendors(query: string): VendorInfo[] {
  const lowercaseQuery = query.toLowerCase()
  return Object.values(vendorDatabase).filter(
    (vendor) =>
