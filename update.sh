...
# chart-fix-update.sh
# Comprehensive chart fix for Total Cost Analyzer
# This script consolidates multiple fixes and resolves chart initialization issues

echo "🔧 Starting Zero Trust TCO Analyzer chart fix installation..."

# Create backup of current state
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
BACKUP_DIR="backup_chart_fix_${TIMESTAMP}"

echo "📦 Creating backup in ${BACKUP_DIR}..."
mkdir -p "${BACKUP_DIR}"
cp js/charts.js "${BACKUP_DIR}/"
cp js/main.js "${BACKUP_DIR}/"
cp index.html "${BACKUP_DIR}/"

# Create consolidated chart fix
cat > js/charts-fix.js << 'EOL'
/**
...
# Update index.html to include the fixed chart script
# First, check if the script is already included
if ! grep -q "charts-fix.js" index.html; then
  echo "📝 Updating index.html to include the fixed chart script..."
  
  # Create a temporary file for the changes
  TMP_FILE=$(mktemp)
  
  # Replace the old script with the new one or add it if it doesn't exist
  if grep -q "charts.js" index.html; then
    # Replace existing charts.js reference
    sed 's|<script src="js/charts.js"></script>|<script src="js/charts.js"></script>\n    <script src="js/charts-fix.js"></script>|' index.html > "$TMP_FILE"
  else
    # Add new script before the closing body tag
    sed 's|</body>|    <script src="js/charts-fix.js"></script>\n</body>|' index.html > "$TMP_FILE"
  fi
...
# Create a simple fix verification script
cat > verify-chart-fix.js << 'EOL'
/**
...
echo "📄 Creating readme for the fix..."
cat > README-CHART-FIX.md << 'EOL'
...
echo "✅ Chart fix installation complete!"
echo "📋 Next steps:"
echo "  1. Test the implementation by loading the application in a browser"
echo "  2. Run the verification script from the browser console"
echo "  3. Check browser console for any remaining errors"
echo "  4. Test all tab navigation to ensure charts load properly"
echo ""
echo "📚 Documentation saved to README-CHART-FIX.md"
