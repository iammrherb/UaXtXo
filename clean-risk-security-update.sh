...
# Clean Risk & Security Update Script - Premium Executive View ONLY
# This ensures we're working with Premium Executive View and not Ultimate Visual View

echo "🔄 Ensuring Premium Executive View is active..."

# First, let's check if we have the correct Premium Executive Platform
if ! grep -q "Premium Executive Platform" js/views/premium-executive-platform.js; then
    echo "❌ Premium Executive Platform not found. Please ensure you've reverted to the correct version."
    echo "Run: git checkout js/views/premium-executive-platform.js"
    exit 1
fi

# Check that we don't have Ultimate Visual View code
if grep -q "UltimateVisualPlatform" js/views/premium-executive-platform.js; then
    echo "❌ Ultimate Visual View code detected! Please revert to Premium Executive View first."
    echo "Run: git checkout HEAD~1 js/views/premium-executive-platform.js"
    exit 1
fi

echo "✅ Premium Executive Platform confirmed"

# Create a backup
cp js/views/premium-executive-platform.js js/views/premium-executive-platform.js.backup
cp css/premium-executive-platform.css css/premium-executive-platform.css.backup

# Create a temporary file with ONLY the renderRiskAssessment implementation
cat > temp_risk_implementation.txt << 'EOF'
...
# Now add all the supporting methods
cat >> temp_risk_implementation.txt << 'EOF'
...
# Use sed to replace the renderRiskAssessment method
echo "📝 Updating renderRiskAssessment method..."
sed -i.bak '/renderRiskAssessment(container) {/,/^    }$/c\
PLACEHOLDER_FOR_NEW_METHOD' js/views/premium-executive-platform.js

# Now insert our new implementation
awk '/PLACEHOLDER_FOR_NEW_METHOD/ {system("cat temp_risk_implementation.txt"); next} 1' js/views/premium-executive-platform.js > temp_updated.js
mv temp_updated.js js/views/premium-executive-platform.js

# Clean up
rm temp_risk_implementation.txt
rm js/views/premium-executive-platform.js.bak

# Add the CSS styles (only the new Risk & Security styles)
echo "🎨 Adding Risk & Security CSS styles..."
...
/* ===== END RISK & SECURITY STYLES ===== */
EOF

echo "✅ Risk & Security implementation complete!"
echo ""
echo "🔍 Verification steps:"
echo "1. Check that Premium Executive Platform is active"
echo "2. No Ultimate Visual View code is present"
echo "3. Risk & Security tab is fully implemented"
echo ""
echo "📦 To commit ONLY these changes:"
echo "git add js/views/premium-executive-platform.js css/premium-executive-platform.css"
echo "git commit -m 'feat: Add Risk & Security analysis to Premium Executive Platform'"
echo "git push"
echo ""
echo "⚠️  If you see any Ultimate Visual View code, DO NOT commit!"
echo "Instead, revert and re-run this script."
