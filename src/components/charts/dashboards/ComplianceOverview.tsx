...
import { useComplianceData, type ComplianceStandard } from "@/hooks/useComplianceData"
import { useIndustryData, type Industry } from "@/hooks/useIndustryData"
...
const getCoverageColor = (coveragePercent: number | undefined, coverageLevel?: ComplianceLevel): string => {
  ...
  if (coveragePercent === undefined) return "bg-slate-700/50 hover:bg-slate-700/70 text-slate-300"
  if (coveragePercent >= 80) return "bg-emerald-500/70 hover:bg-emerald-500/90 text-white"
  if (coveragePercent >= 50) return "bg-yellow-500/70 hover:bg-yellow-500/90 text-gray-800"
  return "bg-red-500/70 hover:bg-red-500/90 text-white"
}
...
const ComplianceHeatmapMatrix: React.FC<ComplianceHeatmapMatrixProps> = ({
  industry,
              ...
              {row.vendorCoverage.map((vc) => (
                <td
                  ...
                  className={cn(
                    ...
                    getCoverageColor(vc.automationPercent, vc.coverageLevel),
                  )}
...
const ComplianceOverview: React.FC = () => {
  ...
  const vendorsForHeatmap = useMemo(() => {
    ...
    return selectedVendorIdsForHeatmap.map((id) => vendorsMap.get(id)).filter(Boolean) as NewVendorData[]
    ...
    <motion.div variants={fadeInUp} initial="initial" animate="animate" className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-lg text-slate-300 max-w-3xl mx-auto">
          Analyze compliance coverage across standards for {currentIndustry?.name || "selected industry"} and compare
          vendor capabilities.
        </p>
      </div>
      ...
      <Card className="bg-slate-800/30 border-slate-700/50 shadow-xl">
        <CardHeader>
          ...
          <CardDescription className="text-slate-400">
            Vendor compliance coverage for standards relevant to the{" "}
            <Badge variant="secondary" className="bg-blue-500/20 text-blue-300 border-blue-400/30">
              {currentIndustry?.name || "N/A"}
            </Badge>{" "}
            industry.
          </CardDescription>
        </CardHeader>
      ...
      <Card className="bg-slate-800/30 border-slate-700/50 shadow-xl">
        <CardHeader>
          <CardTitle className="text-xl text-white">
            Key Standards for {currentIndustry?.name || "Selected Industry"}
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {!isLoading && standardsForSelectedIndustry.length > 0 ? (
              ...
              {standardsForSelectedIndustry.map((standard) => (
                <li key={standard.id} className="p-3 bg-slate-800/50 rounded-lg border border-slate-700">
                  <h4 className="font-semibold text-cyan-400">
                    {standard.name} <span className="text-xs text-slate-400">({standard.id.toUpperCase()})</span>
                  </h4>
            ...
            !isLoading && (
              <p className="text-slate-400">
                No specific standards listed for the selected industry, or select an industry.
              </p>
            )
          )}
        </CardContent>
