...
import { allVendorsData, getVendorDataById, VENDOR_IDS_DEFINITIVE, type VendorId, type NewVendorData } from '@/lib/vendors/data';
...
export function useVendorData() {
  const { data: vendorsMap, isLoading: isLoadingAllVendors, error: errorAllVendors } = useQuery<Map<VendorId, NewVendorData>, Error>({
    queryKey: ['allVendors'],
    queryFn: fetchAllVendors,
    staleTime: Infinity, // Data is static for this app version
  });

  const getVendor = useCallback((id: VendorId): NewVendorData | undefined => {
