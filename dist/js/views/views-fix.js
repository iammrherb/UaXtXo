document.addEventListener("DOMContentLoaded",function(){console.log("Initializing Security and Executive View fixes...");setTimeout(fixViews,700)});function fixViews(){const mainTabs=document.querySelectorAll(".main-tab");if(!mainTabs||mainTabs.length===0){console.warn("No main tabs found, will try again in 1 second");setTimeout(fixViews,1e3);return}console.log(`Found ${mainTabs.length} main tabs, applying fixes`);mainTabs.forEach(tab=>{const viewId=tab.getAttribute("data-view");const newTab=tab.cloneNode(true);tab.parentNode.replaceChild(newTab,tab);newTab.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();console.log(`Switching to view: ${viewId}`);mainTabs.forEach(t=>t.classList.remove("active"));newTab.classList.add("active");const viewPanels=document.querySelectorAll(".view-panel");viewPanels.forEach(panel=>panel.classList.remove("active"));const targetPanel=document.querySelector(`.view-panel[data-view="${viewId}"]`);if(targetPanel){targetPanel.classList.add("active");refreshChartsInView(viewId)}else{console.warn(`View panel not found for ${viewId}`)}})});document.querySelectorAll(".view-panel").forEach(viewPanel=>{const resultsTabs=viewPanel.querySelectorAll(".results-tab");resultsTabs.forEach(tab=>{const panelId=tab.getAttribute("data-panel");const newTab=tab.cloneNode(true);tab.parentNode.replaceChild(newTab,tab);newTab.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();console.log(`Switching to panel: ${panelId}`);resultsTabs.forEach(t=>t.classList.remove("active"));newTab.classList.add("active");const resultsPanels=viewPanel.querySelectorAll(".results-panel");resultsPanels.forEach(panel=>panel.classList.remove("active"));const targetPanel=document.getElementById(panelId);if(targetPanel){targetPanel.classList.add("active");refreshChartsInPanel(panelId)}else{console.warn(`Results panel not found: ${panelId}`)}})})});ensureSecurityViewExists();enhanceExecutiveView();console.log("View fixes applied successfully")}function ensureSecurityViewExists(){let securityPanel=document.querySelector('.view-panel[data-view="security"]');if(!securityPanel){console.log("Security panel does not exist, creating it");const referencePanel=document.querySelector('.view-panel[data-view="financial"]')||document.querySelector('.view-panel[data-view="executive"]');if(!referencePanel){console.warn("No reference panel found, cannot create Security panel");return}securityPanel=document.createElement("div");securityPanel.className="view-panel";securityPanel.setAttribute("data-view","security");securityPanel.innerHTML=`
    ...
    `;referencePanel.parentNode.insertBefore(securityPanel,referencePanel.nextSibling);initializeTabsInPanel(securityPanel);addSecurityTabToMainNavigation()}}function addSecurityTabToMainNavigation(){const mainTabsContainer=document.querySelector(".main-tabs");if(!mainTabsContainer){console.warn("Main tabs container not found, cannot add Security tab");return}const existingSecurityTab=mainTabsContainer.querySelector('.main-tab[data-view="security"]');if(existingSecurityTab)return;const securityTab=document.createElement("button");securityTab.className="main-tab";securityTab.setAttribute("data-view","security");securityTab.innerHTML='<i class="fas fa-shield-alt"></i> Security';securityTab.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();const mainTabs=document.querySelectorAll(".main-tab");mainTabs.forEach(t=>t.classList.remove("active"));securityTab.classList.add("active");const viewPanels=document.querySelectorAll(".view-panel");viewPanels.forEach(panel=>panel.classList.remove("active"));const securityPanel=document.querySelector('.view-panel[data-view="security"]');if(securityPanel){securityPanel.classList.add("active");refreshChartsInView("security")}});mainTabsContainer.appendChild(securityTab)}function enhanceExecutiveView(){const executivePanel=document.querySelector('.view-panel[data-view="executive"]');if(!executivePanel){console.warn("Executive panel not found, cannot enhance");return}const summaryPanel=executivePanel.querySelector("#executive-summary");if(!summaryPanel){console.warn("Executive summary panel not found, cannot enhance");return}if(summaryPanel.querySelector(".analyst-quotes"))return;const quotesContainer=document.createElement("div");quotesContainer.className="chart-container analyst-quotes";quotesContainer.innerHTML=`
  ...
  `;document.head.appendChild(style);summaryPanel.appendChild(quotesContainer);const comparisonPanel=executivePanel.querySelector("#executive-comparison");if(comparisonPanel){if(comparisonPanel.querySelector(".vendor-matrix"))return;const matrixContainer=document.createElement("div");matrixContainer.className="chart-container vendor-matrix";matrixContainer.innerHTML=`
    ...
    `;const tableStyle=document.createElement("style");tableStyle.textContent=`
    ...
    `;document.head.appendChild(tableStyle);comparisonPanel.appendChild(matrixContainer)}}function initializeTabsInPanel(panel){const tabs=panel.querySelectorAll(".results-tab");const panels=panel.querySelectorAll(".results-panel");tabs.forEach(tab=>{const panelId=tab.getAttribute("data-panel");tab.addEventListener("click",function(){tabs.forEach(t=>t.classList.remove("active"));tab.classList.add("active");panels.forEach(p=>p.classList.remove("active"));const targetPanel=document.getElementById(panelId);if(targetPanel){targetPanel.classList.add("active");refreshChartsInPanel(panelId)}})})}function refreshChartsInView(viewId){console.log(`Refreshing charts in view: ${viewId}`);const viewPanel=document.querySelector(`.view-panel[data-view="${viewId}"]`);if(!viewPanel)return;const activeResultsPanel=viewPanel.querySelector(".results-panel.active");if(!activeResultsPanel)return;const panelId=activeResultsPanel.id;refreshChartsInPanel(panelId)}function refreshChartsInPanel(panelId){console.log(`Refreshing charts in panel: ${panelId}`);const chartContainers=document.querySelectorAll(`#${panelId} .chart-wrapper`);chartContainers.forEach(container=>{if(window.apexChartManager&&typeof window.apexChartManager.refreshChart==="function"){window.apexChartManager.refreshChart(container.id)}else if(window.d3Manager&&typeof window.d3Manager.refreshChart==="function"){window.d3Manager.refreshChart(container.id)}})}