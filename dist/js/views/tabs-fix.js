document.addEventListener("DOMContentLoaded",function(){console.log("Applying tabs fix...");setTimeout(fixTabs,500)});function fixTabs(){const mainTabsContainers=document.querySelectorAll(".main-tabs");if(mainTabsContainers.length>1){console.log(`Found ${mainTabsContainers.length} main-tabs containers, fixing...`);for(let i=1;i<mainTabsContainers.length;i++){mainTabsContainers[i].remove()}}else if(mainTabsContainers.length===0){console.log("No main-tabs container found, creating one...");const mainTabsContainer=document.createElement("div");mainTabsContainer.className="main-tabs";const contentArea=document.querySelector(".content-area");if(contentArea){const contentWrapper=contentArea.querySelector(".content-wrapper")||contentArea;contentWrapper.prepend(mainTabsContainer)}}const mainTabsContainer=document.querySelector(".main-tabs");if(!mainTabsContainer){console.error("Main tabs container still not found after fix attempt");return}mainTabsContainer.innerHTML="";const requiredViews=[{id:"executive",icon:"chart-line",label:"Executive"},{id:"financial",icon:"dollar-sign",label:"Financial"},{id:"security",icon:"shield-alt",label:"Security"},{id:"technical",icon:"cogs",label:"Technical"}];requiredViews.forEach((view,index)=>{const tab=document.createElement("button");tab.className="main-tab"+(index===0?" active":"");tab.setAttribute("data-view",view.id);tab.innerHTML=`<i class="fas fa-${view.icon}"></i> ${view.label}`;mainTabsContainer.appendChild(tab)});requiredViews.forEach((view,index)=>{let viewPanel=document.querySelector(`.view-panel[data-view="${view.id}"]`);if(!viewPanel){console.log(`Creating missing view panel for ${view.id}`);viewPanel=document.createElement("div");viewPanel.className="view-panel"+(index===0?" active":"");viewPanel.setAttribute("data-view",view.id);viewPanel.innerHTML=`
        <div class="results-tabs">
          <button class="results-tab active" data-panel="${view.id}-summary">${view.label} Summary</button>
          ${view.id==="executive"?`<button class="results-tab" data-panel="executive-roi">ROI Analysis</button>
          ...
          ${view.id==="financial"?`<button class="results-tab" data-panel="financial-tco">TCO Analysis</button>
          ...
          ${view.id==="security"?`<button class="results-tab" data-panel="security-overview">Security Overview</button>
          ...
          ${view.id==="technical"?`<button class="results-tab" data-panel="technical-features">Features</button>
        ...
        <div id="${view.id}-summary" class="results-panel active">
          ...
          ${view.id==="executive"?`<div class="dashboard-grid">
          ...
          ${view.id==="security"?`<div class="dashboard-grid">
              <div class="dashboard-card highlight-card">
        ...
        ${view.id==="executive"?`<!-- Executive ROI Panel -->
        ...
        ${view.id==="security"?`<!-- Security Overview -->
      ...
      `;const contentArea=document.querySelector(".content-area");if(contentArea){const contentWrapper=contentArea.querySelector(".content-wrapper")||contentArea;contentWrapper.appendChild(viewPanel)}}});document.querySelectorAll(".main-tab").forEach(tab=>{tab.addEventListener("click",function(){const viewId=this.getAttribute("data-view");document.querySelectorAll(".main-tab").forEach(t=>t.classList.remove("active"));this.classList.add("active");document.querySelectorAll(".view-panel").forEach(panel=>panel.classList.remove("active"));const viewPanel=document.querySelector(`.view-panel[data-view="${viewId}"]`);if(viewPanel){viewPanel.classList.add("active")}})});document.querySelectorAll(".results-tab").forEach(tab=>{tab.addEventListener("click",function(){const panelId=this.getAttribute("data-panel");const viewPanel=this.closest(".view-panel");if(!viewPanel)return;viewPanel.querySelectorAll(".results-tab").forEach(t=>t.classList.remove("active"));this.classList.add("active");viewPanel.querySelectorAll(".results-panel").forEach(panel=>panel.classList.remove("active"));const resultsPanel=document.getElementById(panelId);if(resultsPanel){resultsPanel.classList.add("active")}})});console.log("Tabs fix applied successfully")}