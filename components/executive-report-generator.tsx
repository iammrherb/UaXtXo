...
import { FileText, Download, Share2, CheckCircle2, TrendingUp } from "lucide-react"
...
export function ExecutiveReportGenerator({
  ...
  const industryROI = INDUSTRY_ROI[industry]

  // Generate report content based on type
  const reportContent = useMemo(() => {
    const portnoxData = comparisonData["PORTNOX"]
    const competitorAvg =
      Object.entries(comparisonData)
        .filter(([key]) => key !== "PORTNOX")
        .reduce((acc, [_, data]: [string, any]) => acc + data.totalCost, 0) /
          ...
          sections: [
            {
              ...
              breakdown: {
                "Software Licensing": { portnox: portnoxData.software, competitors: competitorAvg * 0.4 },
                "Hardware/Infrastructure": { portnox: 0, competitors: competitorAvg * 0.3 },
                Implementation: { portnox: portnoxData.implementation, competitors: competitorAvg * 0.2 },
                Operations: { portnox: portnoxData.operational, competitors: competitorAvg * 0.1 },
              },
            },
            {
  ...
  }, [reportType, industry, deviceCount, timeframe, comparisonData, roiData])
