    ...
    return (
      ...
      >
        ...
        >
          ...
          <CarouselControls next={scrollNext} prev={scrollPrev} />
        </div>
      </CarouselContext.Provider>
    )
  },
...
interface CarouselControlsProps extends React.HTMLAttributes<HTMLDivElement> {
  next: () => void
  prev: () => void
}

const CarouselControls = React.forwardRef<HTMLDivElement, CarouselControlsProps>(
  ({ className, next, prev, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn("absolute bottom-4 left-0 right-0 flex justify-center space-x-2", className)}
        {...props}
      >
