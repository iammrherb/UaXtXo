...
import { TrendingUp, AlertTriangle, Shield, CheckCircle, Settings } from "lucide-react"
...
const ENHANCED_VENDOR_COSTS = {
  ...
  fortinet: {
    ...
    professionalServices: {
      ...
      migration: { cost: 65000, duration: 10, description: "Migration from legacy solution" },
      training: {
        virtual: { cost: 2000, duration: 2, participants: 12 },
        onsite: { cost: 4500, duration: 3, participants: 16 },
        certification: { cost: 1500, duration: 2, participants: 1 },
      },
    },
    operationalMetrics: {
      automationLevel: 70,
  ...
  forescout: {
    licensing: {
      ...
      tiers: {
        base: { multiplier: 1.0, features: ["Device visibility", "Basic classification", "Policy enforcement"] },
        advanced: { multiplier: 1.3, features: ["Advanced classification", "Threat detection", "Orchestration"] },
        enterprise: { multiplier: 1.6, features: ["Full platform", "AI/ML", "Extended modules"] },
      },
    },
    addOns: {
      extendedModules: { cost: 30, perDevice: true, description: "Additional security and compliance modules" },
      threatDetection: { cost: 25, perDevice: true, description: "Advanced threat detection and response" },
      orchestration: { cost: 20, perDevice: true, description: "Security orchestration and automation" },
      complianceModule: { cost: 15, perDevice: true, description: "Regulatory compliance monitoring" },
    },
    integrations: {
      securityTools: { cost: 0, complexity: "medium", timeToImplement: 8 },
      siem: { cost: 0, complexity: "medium", timeToImplement: 10 },
      activeDirctory: { cost: 0, complexity: "medium", timeToImplement: 12 },
      vmware: { cost: 5000, complexity: "high", timeToImplement: 16 },
      aws: { cost: 8000, complexity: "high", timeToImplement: 20 },
    },
    hardware: {
      required: true,
      appliances: [
        { model: "CT1000", capacity: 2000, cost: 25000, redundancy: true },
        { model: "CT3000", capacity: 10000, cost: 65000, redundancy: true },
        { model: "CT5000", capacity: 50000, cost: 125000, redundancy: true },
      ],
    },
    infrastructure: {
      serverRequirements: "VMware vSphere 6.5+, 16GB RAM minimum",
      networkRequirements: "Network TAPs or SPAN ports",
      storageRequirements: "1TB SSD minimum",
      backupRequirements: "Customer managed",
    },
    support: {
      basic: { cost: 15000, coverage: "8x5 Support", sla: "Next business day" },
      premium: { cost: 28000, coverage: "24x7 Support", sla: "4 hours" },
      enterprise: { cost: 45000, coverage: "24x7 Priority + TAM", sla: "2 hours" },
    },
    professionalServices: {
      quickStart: { cost: 20000, duration: 4, description: "Basic CounterACT setup" },
      standardImplementation: { cost: 65000, duration: 10, description: "Full platform deployment" },
      enterpriseDeployment: { cost: 150000, duration: 20, description: "Complex multi-site deployment" },
      migration: { cost: 120000, duration: 16, description: "Migration and integration" },
      training: {
        virtual: { cost: 3500, duration: 3, participants: 8 },
        onsite: { cost: 7500, duration: 4, participants: 12 },
        certification: { cost: 2800, duration: 4, participants: 1 },
      },
    },
    operationalMetrics: {
      automationLevel: 80,
      fteSavingsPerDevice: 0.0006,
      maintenanceHoursPerMonth: 18,
      upgradeComplexity: "high",
      troubleshootingTimeHours: 5,
    },
  },
}
...
export default function IndustryAnalysisView({ results, config }: IndustryAnalysisViewProps) {
  ...
  const [selectedThreatModel, setSelectedThreatModel] = useState<string>("current")
  const [comparisonMode, setComparisonMode] = useState<string>("single")
  const [selectedIndustries, setSelectedIndustries] = useState<string[]>(["healthcare", "financial"])
  const [riskTolerance, setRiskTolerance] = useState<number>(50)
  const [budgetConstraint, setBudgetConstraint] = useState<number>(1000000)
  const [complianceWeight, setComplianceWeight] = useState<number>(80)
  const [operationalWeight, setOperationalWeight] = useState<number>(60)
  const [securityWeight, setSecurityWeight] = useState<number>(90)
  const [showAdvancedMetrics, setShowAdvancedMetrics] = useState<boolean>(false)
  const [customSettings, setCustomSettings] = useState({
    deviceCount: config.deviceCount || 5000,
    ...
    includeProfessionalServices: true,
    portnoxPerDeviceCost: 60,
    portnoxTier: "professional",
    portnoxAddOns: ["riskAnalytics", "complianceReporting"],
  })

  // Calculate enhanced industry analysis with detailed metrics
  const industryAnalysis = useMemo(() => {
    ...
    const threats = THREAT_MODELS[selectedIndustry as keyof typeof THREAT_MODELS]?.threats || []
    
    // Calculate compliance costs and benefits
    const relevantFrameworks = Object.entries(COMPLIANCE_FRAMEWORKS).filter(([_, framework]) =>
      framework.industry.includes(selectedIndustry) || selectedFramework === "all"
    )
    
    const complianceCosts = relevantFrameworks.reduce((total, [_, framework]) => {
      return total + Object.values(framework.controls).reduce((sum, control) => 
        sum + control.implementationCost + (control.ongoingCost * 3), 0
      )
    }, 0)
    
    const complianceBenefits = relevantFrameworks.reduce((total, [_, framework]) => {
      return total + Object.values(framework.controls).reduce((sum, control) => 
        sum + (control.riskReduction * 10000), 0
      )
    }, 0)
    
    // Calculate threat-based risk reduction
    const threatRiskReduction = threats.reduce((total, threat) => {
      const riskValue = threat.probability * threat.impact
      const mitigatedRisk = riskValue * (threat.nacMitigation / 100)
      return total + mitigatedRisk
    }, 0)
    
    // Calculate operational efficiency gains
    const operationalSavings = customSettings.deviceCount * currentIndustry.operationalMetrics.avgFtePerDevice * 120000 // avg FTE cost
    const maintenanceSavings = customSettings.deviceCount * currentIndustry.operationalMetrics.maintenanceHoursPerDevice * 150 * 12 // hourly rate
    
    // Enhanced vendor cost calculation with Portnox adjustments
    const portnoxCosts = calculateEnhancedVendorCosts("portnox", customSettings)
    const ciscoCosts = calculateEnhancedVendorCosts("cisco", customSettings)
    const arubaCosts = calculateEnhancedVendorCosts("aruba", customSettings)
    ...
    return {
      ...
      totalRoi: (complianceBenefits + threatRiskReduction + operationalSavings + maintenanceSavings) / (complianceCosts + portnoxCosts.totalCost),
    }
  }, [selectedIndustry, selectedFramework, customSettings])

  // Enhanced vendor cost calculation function
  function calculateEnhancedVendorCosts(vendorKey: string, settings: any) {
    const vendor = ENHANCED_VENDOR_COSTS[vendorKey as keyof typeof ENHANCED_VENDOR_COSTS]
    if (!vendor) return { totalCost: 0, breakdown: {} }
    
    const deviceCount = settings.deviceCount
    const timeframe = settings.timeframe
    
    // Base licensing cost with volume discounts
    let baseCost = vendor.licensing.perDeviceBase
    const volumeDiscount = Object.entries(vendor.licensing.volumeDiscounts)
      .reverse()
      .find(([threshold]) => deviceCount >= Number.parseInt(threshold))?.[1] || 1
    
    baseCost *= volumeDiscount
    
    // Tier multiplier
    const tierMultiplier = vendor.licensing.tiers[settings[`${vendorKey}Tier`] as keyof typeof vendor.licensing.tiers]?.multiplier || 1
    baseCost *= tierMultiplier
    
    const licensingCost = baseCost * deviceCount * (timeframe / 12)
    
    // Add-ons cost
    const addOnsCost = settings[`${vendorKey}AddOns`]?.reduce((total: number, addOnKey: string) => {
      const addOn = vendor.addOns[addOnKey as keyof typeof vendor.addOns]
      if (!addOn) return total
      
      if (addOn.perDevice) {
        return total + (addOn.cost * deviceCount * (timeframe / 12))
      } else if (addOn.perUser) {
        return total + (addOn.cost * Math.ceil(deviceCount * 0.1) * (timeframe / 12))
      } else {
        return total + (addOn.cost * (timeframe / 12))
      }
    }, 0) || 0
    
    // Hardware cost (if required)
    const hardwareCost = vendor.hardware.required ? 
      calculateHardwareCost(vendor.hardware, deviceCount) : 0
    
    // Support cost
    const supportCost = vendor.support.premium.cost * (timeframe / 12)
    
    // Professional services
    const professionalServicesCost = vendor.professionalServices.standardImplementation.cost
    
    // Integration costs
    const integrationCost = settings.includeIntegrations ? 
      Object.values(vendor.integrations).reduce((total: number, integration: any) => 
        total + (integration.cost || 0), 0
      ) : 0
    
    const totalCost = licensingCost + addOnsCost + hardwareCost + supportCost + 
                     professionalServicesCost + integrationCost
    ...
    return {
      ...
      operationalMetrics: vendor.operationalMetrics,
    }
  }
  
  function calculateHardwareCost(hardware: any, deviceCount: number) {
    if (!hardware.appliances) return 0
    
    const requiredAppliance = hardware.appliances
      .find((appliance: any) => appliance.capacity >= deviceCount) || 
      hardware.appliances[hardware.appliances.length - 1]
    
    const applianceCount = Math.ceil(deviceCount / requiredAppliance.capacity)
    const redundancyMultiplier = requiredAppliance.redundancy ? 2 : 1
    
    return requiredAppliance.cost * applianceCount * redundancyMultiplier
  }

  // Multi-industry comparison data
  const multiIndustryComparison = useMemo(() => {
    if (comparisonMode !== "multi") return null
    
    return selectedIndustries.map(industry => {
      const benchmark = INDUSTRY_BENCHMARKS[industry as keyof typeof INDUSTRY_BENCHMARKS]
      const threats = THREAT_MODELS[industry as keyof typeof THREAT_MODELS]?.threats || []
      
      return {
        industry,
        benchmark,
        threats,
        avgRisk: threats.reduce((sum, threat) => sum + (threat.probability * threat.impact), 0) / threats.length,
        complianceComplexity: benchmark.complianceRequirement,
        securityMaturity: benchmark.maturityLevel,
      }
    })
  }, [comparisonMode, selectedIndustries])
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-2">
              <Label htmlFor="industry-select">Primary Industry</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="comparison-mode">Analysis Mode</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="device-count">Device Count</Label>
              <Input
                type="number"
                value={customSettings.deviceCount}
                onChange={(e) => setCustomSettings(prev => ({
                  ...prev,
                  deviceCount: Number.parseInt(e.target.value) || 0
                }))}
                min="100"
                max="100000"
                step="100"
          ...
          {/* Portnox-specific settings */}
          <div className="border rounded-lg p-4 space-y-4">
            <h4 className="font-semibold text-sm flex items-center gap-2">
              <img src="/portnox-logo.png" alt="Portnox" className="h-4 w-4" />
              Portnox Configuration
            </h4>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label>Per-Device Cost ($)</Label>
                <Slider
                  value={[customSettings.portnoxPerDeviceCost]}
                  onValueChange={([value]) => setCustomSettings(prev => ({
                    ...prev,
                    portnoxPerDeviceCost: value
                  }))}
                  min={40}
                  max={100}
                  step={5}
                ...
                <div className="text-sm text-muted-foreground">
                  ${customSettings.portnoxPerDeviceCost}/device/year
                </div>
              </div>
              
              <div className="space-y-2">
                <Label>Portnox Tier</Label>
                <Select 
                  value={customSettings.portnoxTier} 
                  onValueChange={(value) => setCustomSettings(prev => ({
                    ...prev,
                    portnoxTier: value
                  }))}
                >
                  ...
                  <SelectContent>
                    <SelectItem value="essentials">Essentials (75% base cost)</SelectItem>
                    <SelectItem value="professional">Professional (100% base cost)</SelectItem>
                    <SelectItem value="enterprise">Enterprise (133% base cost)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  {Object.entries(ENHANCED_VENDOR_COSTS.portnox.addOns).map(([key, addon]) => (
                    <div key={key} className="flex items-center space-x-2">
                      <Switch
                        checked={customSettings.portnoxAddOns.includes(key)}
                        onCheckedChange={(checked) => {
                          setCustomSettings(prev => ({
                            ...prev,
                            portnoxAddOns: checked 
                              ? [...prev.portnoxAddOns, key]
                              : prev.portnoxAddOns.filter(a => a !== key)
                          }))
                        }}
                      />
                      <Label className="text-xs">{key} (+${addon.cost})</Label>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label>Security Weight ({securityWeight}%)</Label>
              <Slider
                value={[securityWeight]}
                onValueChange={([value]) => setSecurityWeight(value)}
                min={0}
                max={100}
                step={5}
              />
            ...
            <div className="space-y-2">
              <Label>Compliance Weight ({complianceWeight}%)</Label>
              <Slider
                value={[complianceWeight]}
                onValueChange={([value]) => setComplianceWeight(value)}
                min={0}
                max={100}
                step={5}
              />
            ...
            <div className="space-y-2">
              <Label>Operational Weight ({operationalWeight}%)</Label>
              <Slider
                value={[operationalWeight]}
                onValueChange={([value]) => setOperationalWeight(value)}
                min={0}
                max={100}
                step={5}
              />
      ...
      <Tabs defaultValue="overview" className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{industryAnalysis.industry.maturityLevel}%</p>
                  </div>
                  <Shield className="h-8 w-8 text-blue-500" />
                </div>
                <Progress value={industryAnalysis.industry.maturityLevel} className="mt-2" />
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{industryAnalysis.industry.complianceRequirement}%</p>
                  </div>
                  <CheckCircle className="h-8 w-8 text-green-500" />
                </div>
                <Progress value={industryAnalysis.industry.complianceRequirement} className="mt-2" />
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Avg Breach Cost</p>
                    <p className="text-2xl font-bold">${(industryAnalysis.industry.avgBreachCost / 1000000).toFixed(1)}M</p>
                  </div>
                  <AlertTriangle className="h-8 w-8 text-red-500" />
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  {(industryAnalysis.industry.breachFrequency * 100).toFixed(1)}% annual probability
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">ROI Projection</p>
                    <p className="text-2xl font-bold">{(industryAnalysis.totalRoi * 100).toFixed(0)}%</p>
                  </div>
            ...
            <CardContent>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart data={[
                    { subject: 'Security Maturity', A: industryAnalysis.industry.maturityLevel, fullMark: 100 },
                    { subject: 'Compliance', A: industryAnalysis.industry.complianceRequirement, fullMark: 100 },
                    { subject: 'Automation', A: industryAnalysis.industry.automationLevel, fullMark: 100 },
                    { subject: 'Cloud Adoption', A: industryAnalysis.industry.cloudAdoption, fullMark: 100 },
                    { subject: 'NAC Adoption', A: industryAnalysis.industry.nacAdoption, fullMark: 100 },
                    { subject: 'Budget Growth', A: industryAnalysis.industry.budgetGrowth * 5, fullMark: 100 },
                  ]}>
                    <PolarGrid />
                    <PolarAngleAxis dataKey="subject" />
                    <PolarRadiusAxis angle={90} domain={[0, 100]} />
                    <Radar name="Current Industry" dataKey="A" stroke="#8884d8" fill="#8884d8" fillOpacity={0.6} />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
        ...
        <TabsContent value="compliance" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Compliance Framework Requirements</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {industryAnalysis.frameworks.map(([key, framework]) => (
                    <div key={key} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-semibold">{framework.name}</h4>
                        <Badge variant={framework.maxFine > 1000000 ? "destructive" : "secondary"}>
                          Max Fine: ${(framework.maxFine / 1000000).toFixed(1)}M
                        </Badge>
                      </div>
                      <p className="text-sm text-muted-foreground mb-3">{framework.fullName}</p>
                      
                      <div className="space-y-2">
                        {Object.entries(framework.controls).slice(0, 3).map(([controlKey, control]) => (
                          <div key={controlKey} className="flex items-center justify-between text-sm">
                            <span className="font-medium">{control.name}</span>
                            <div className="flex items-center gap-2">
                              <Badge variant={control.nacRequirement === "critical" ? "destructive" : 
                                           control.nacRequirement === "important" ? "default" : "secondary"}>
                                {control.nacRequirement}
                              </Badge>
                              <span className="text-muted-foreground">{control.complianceScore}%</span>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Compliance Investment Analysis</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 bg-red-50 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-red-600">
                        ${(industryAnalysis.complianceCosts / 1000000).toFixed(1)}M
                      </p>
                    </div>
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      <p className="text-sm text-muted-foreground">Risk Reduction Value</p>
                      <p className="text-2xl font-bold text-green-600">
                        ${(industryAnalysis.complianceBenefits / 1000000).toFixed(1)}M
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={industryAnalysis.frameworks.map(([key, framework]) => ({
                        name: framework.name,
                        cost: Object.values(framework.controls).reduce((sum, control) => 
                          sum + control.implementationCost, 0) / 1000,
                        benefit: Object.values(framework.controls).reduce((sum, control) => 
                          sum + (control.riskReduction * 100), 0),
                      }))}>
                        ...
                        <Bar dataKey="benefit" fill="#22c55e" name="Benefit ($K)" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Detailed Control Requirements</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {industryAnalysis.frameworks.map(([frameworkKey, framework]) => (
                  <div key={frameworkKey}>
                    <h4 className="font-semibold mb-3">{framework.name} Controls</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {Object.entries(framework.controls).map(([controlKey, control]) => (
                        <div key={controlKey} className="border rounded-lg p-4">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <h5 className="font-medium">{control.name}</h5>
                              <p className="text-xs text-muted-foreground">{controlKey}</p>
                            </div>
                            <Badge variant={control.nacRequirement === "critical" ? "destructive" : 
                                         control.nacRequirement === "important" ? "default" : "secondary"}>
                              {control.nacRequirement}
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="font-medium">${control.implementationCost.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span>Annual Cost:</span>
                              <span className="font-medium">${control.ongoingCost.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span>Risk Reduction:</span>
                              <span className="font-medium">{control.riskReduction}%</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span>Compliance Score:</span>
                              <span className="font-medium">{control.complianceScore}%</span>
                            </div>
                          </div>
                          
                          <div className="mt-3">
                            <p className="text-xs font-medium mb-1">NAC Capabilities Required:</p>
                            <div className="flex flex-wrap gap-1">
                              {control.nacCapabilities.map((capability, idx) => (
        ...
        <TabsContent value="threats" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Threat Landscape Analysis</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart data={industryAnalysis.threats.map(threat => ({
                      name: threat.name,
                      probability: threat.probability * 100,
                      impact: threat.impact / 1000000,
                      mitigation: threat.nacMitigation,
                    }))}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="probability" name="Probability (%)" />
                      <YAxis dataKey="impact" name="Impact ($M)" />
                      <Tooltip 
                          ...
                          if (active && payload && payload.length) {
                            ...
                            return (
                              <div className="bg-white p-3 border rounded-lg shadow-lg">
                                ...
                                <p>Impact: ${data.impact.toFixed(1)}M</p>
                                <p>NAC Mitigation: {data.mitigation}%</p>
                              </div>
                            )
                          }
                          return null
                      ...
                      <Scatter dataKey="impact" fill="#8884d8">
                        {industryAnalysis.threats.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={
                            entry.nacMitigation > 80 ? "#22c55e" :
                            entry.nacMitigation > 60 ? "#eab308" : "#ef4444"
                          } />
                        ))}
                      </Scatter>
                    </ScatterChart>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {industryAnalysis.threats.map((threat, index) => (
                    <div key={index} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-semibold">{threat.name}</h4>
                        <Badge variant={threat.nacMitigation > 80 ? "default" : "destructive"}>
                          {threat.nacMitigation}% Mitigation
                        </Badge>
                      </div>
                      
                      <div className="grid grid-cols-4 gap-2 text-sm">
                        <div className="text-center p-2 bg-red-50 rounded">
                          <p className="font-medium">{threat.timeToDetect}h</p>
                          <p className="text-xs text-muted-foreground">Detect</p>
                        </div>
                        <div className="text-center p-2 bg-yellow-50 rounded">
                          <p className="font-medium">{threat.timeToContain}h</p>
                          <p className="text-xs text-muted-foreground">Contain</p>
                        </div>
                        <div className="text-center p-2 bg-blue-50 rounded">
                          <p className="font-medium">{threat.recoveryTime}h</p>
                          <p className="text-xs text-muted-foreground">Recover</p>
                        </div>
                        <div className="text-center p-2 bg-purple-50 rounded">
                          <p className="font-medium">${(threat.regulatoryFines / 1000000).toFixed(1)}M</p>
                          <p className="text-xs text-muted-foreground">Max Fine</p>
                        </div>
                      </div>
                      
                      <div className="mt-3">
                        <p className="text-xs font-medium mb-1">Attack Vectors:</p>
                        <div className="flex flex-wrap gap-1">
          ...
          <Card>
            <CardHeader>
              <CardTitle>NAC Threat Mitigation Effectiveness</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={industryAnalysis.threats.map(threat => ({
                    name: threat.name.split(' ')[0],
                    baseline: threat.probability * threat.impact / 1000000,
                    withNac: (threat.probability * threat.impact * (1 - threat.nacMitigation / 100)) / 1000000,
                    reduction: (threat.probability * threat.impact * (threat.nacMitigation / 100)) / 1000000,
                  }))}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" />
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="baseline" stackId="a" fill="#ef4444" name="Baseline Risk ($M)" />
                    <Bar dataKey="withNac" stackId="a" fill="#eab308" name="Residual Risk ($M)" />
                    <Bar dataKey="reduction" stackId="b" fill="#22c55e" name="Risk Reduction ($M)" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
        ...
        <TabsContent value="operations" className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 bg-blue-50 rounded-lg">
                      <p className="text-sm text-muted-foreground">FTE Savings</p>
                      <p className="text-2xl font-bold text-blue-600">
                        {(industryAnalysis.operationalSavings / 120000).toFixed(1)}
                      </p>
                      <p className="text-xs text-muted-foreground">Full-time equivalents</p>
                    </div>
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      <p className="text-sm text-muted-foreground">Annual Savings</p>
                      <p className="text-2xl font-bold text-green-600">
                        ${(industryAnalysis.operationalSavings / 1000000).toFixed(1)}M
                      </p>
                      <p className="text-xs text-muted-foreground">Operational cost reduction</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="font-semibold">Key Operational Metrics</h4>
                    {Object.entries(industryAnalysis.industry.operationalMetrics).map(([key, value]) => (
                      <div key={key} className="flex justify-between items-center">
                        <span className="text-sm capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}:</span>
                        <span className="font-medium">
                          {typeof value === 'number' ? 
                            (key.includes('Hours') || key.includes('Days') ? `${value} ${key.includes('Hours') ? 'hrs' : 'days'}` :
                             key.includes('Frequency') ? `${value} months` :
                             key.includes('Fte') ? value.toFixed(4) :
                             value.toString()) : 
                            value}
                        </span>
                      </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      {Object.entries(industryAnalysis.industry.infrastructureRequirements).map(([key, value]) => (
                  ...
                  <div>
                    <h4 className="font-semibold mb-2">Vendor Requirements</h4>
                    <div className="space-y-2">
                      {Object.entries(industryAnalysis.industry.vendorRequirements).map(([key, value]) => (
                        <div key={key} className="flex justify-between items-center text-sm">
                          <span className="capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}:</span>
                          <span className="font-medium">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Staffing Impact Analysis</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{industryAnalysis.industry.securityStaffing} FTE</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm">Training Days/Year:</span>
                      <span className="font-medium">{industryAnalysis.industry.operationalMetrics.trainingDaysPerYear}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm">Maintenance Hours:</span>
                      <span className="font-medium">{industryAnalysis.industry.operationalMetrics.maintenanceHoursPerDevice}/device</span>
                    </div>
                  </div>
                </div>
                
                <div className="space-y-4">
                  <h4 className="font-semibold">With NAC Implementation</h4>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-sm">Reduced Staff Need:</span>
                      <span className="font-medium text-green-600">
                        -{(customSettings.deviceCount * industryAnalysis.industry.operationalMetrics.avgFtePerDevice).toFixed(1)} FTE
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm">Automation Level:</span>
                      <span className="font-medium">{industryAnalysis.industry.automationLevel}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm">Efficiency Gain:</span>
                      <span className="font-medium text-green-600">
                        {((industryAnalysis.operationalSavings + industryAnalysis.maintenanceSavings) / 1000000).toFixed(1)}M/year
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h4 className="font-semibold">ROI Breakdown</h4>
                  <div className="h-48">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={[
                            { name: 'Staff Savings', value: industryAnalysis.operationalSavings },
                            { name: 'Maintenance Savings', value: industryAnalysis.maintenanceSavings },
                            { name: 'Compliance Benefits', value: industryAnalysis.complianceBenefits },
                            { name: 'Risk Reduction', value: industryAnalysis.threatRiskReduction },
                          ]}
                          cx="50%"
                          cy="50%"
                          outerRadius={60}
        ...
        <TabsContent value="costs" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={Object.entries(industryAnalysis.vendorCosts).map(([vendor, costs]) => ({
                      vendor: vendor.charAt(0).toUpperCase() + vendor.slice(1),
                      total: costs.totalCost / 1000000,
                      licensing: costs.breakdown.licensing / 1000000,
                      hardware: costs.breakdown.hardware / 1000000,
                      services: costs.breakdown.professionalServices / 1000000,
                      support: costs.breakdown.support / 1000000,
                    }))}>
                      <CartesianGrid strokeDasharray="3 3" />
                      ...
                      <Bar dataKey="services" stackId="a" fill="#eab308" name="Services" />
                      <Bar dataKey="support" stackId="a" fill="#22c55e" name="Support" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {Object.entries(industryAnalysis.vendorCosts.portnox.breakdown).map(([category, cost]) => (
                  ...
                  <div className="flex justify-between items-center font-semibold">
                    ...
                    <span>${(industryAnalysis.vendorCosts.portnox.totalCost / 1000000).toFixed(2)}M</span>
                  </div>
                  
                  <div className="flex justify-between items-center text-sm text-muted-foreground">
                    <span>Per Device (3 years):</span>
                    <span>${(industryAnalysis.vendorCosts.portnox.totalCost / customSettings.deviceCount).toFixed(0)}</span>
                  </div>
                  
                  <div className="flex justify-between items-center text-sm text-muted-foreground">
                    <span>Per Device (Annual):</span>
                    <span>${(industryAnalysis.vendorCosts.portnox.totalCost / customSettings.deviceCount / 3).toFixed(0)}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {Object.entries(industryAnalysis.vendorCosts).map(([vendor, costs]) => (
                  <div key={vendor} className="border rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      />
                      <h4 className="font-semibold capitalize">{vendor}</h4>
                    </div>
                    
                    <div className="space-y-3">
                      <div className="text-center p-3 bg-gray-50 rounded">
                        <p className="text-sm text-muted-foreground">3-Year TCO</p>
                        <p className="text-xl font-bold">${(costs.totalCost / 1000000).toFixed(2)}M</p>
                      </div>
                      
                      <div className="space-y-2 text-sm">
                        {Object.entries(costs.breakdown).map(([category, cost]) => (
                      ...
                      {costs.operationalMetrics && (
                        <div className="pt-2 border-t">
                          <p className="text-xs font-medium mb-1">Operational Impact:</p>
                          <div className="space-y-1 text-xs">
                            <div className="flex justify-between">
                              <span>Automation:</span>
                              <span>{costs.operationalMetrics.automationLevel}%</span>
                            </div>
                            <div className="flex justify-between">
                              <span>FTE Savings:</span>
                              <span>{(costs.operationalMetrics.fteSavingsPerDevice * customSettings.deviceCount).toFixed(1)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Maintenance:</span>
                              <span>{costs.operationalMetrics.maintenanceHoursPerMonth}h/mo</span>
                            </div>
                          </div>
                        </div>
                      )}
        ...
        <TabsContent value="benchmarks" className="space-y-6">
          {comparisonMode === "multi" && multiIndustryComparison ? (
            <Card>
              <CardHeader>
                <CardTitle>Multi-Industry Comparison</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart data={multiIndustryComparison.map(item => ({
                      industry: item.benchmark.name,
                      security: item.benchmark.maturityLevel,
                      compliance: item.benchmark.complianceRequirement,
                      automation: item.benchmark.automationLevel,
                      nacAdoption: item.benchmark.nacAdoption,
                      cloudAdoption: item.benchmark.cloudAdoption,
                    }))}>
                      <PolarGrid />
                      <PolarAngleAxis dataKey="industry" />
                      <PolarRadiusAxis angle={90} domain={[0, 100]} />
                      {multiIndustryComparison.map((item, index) => (
                        <Radar
                          key={item.industry}
                          name={item.benchmark.name}
                          dataKey={item.industry === selectedIndustry ? "security" : "compliance"}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <CardContent>
                  <div className="space-y-4">
                    {Object.entries(INDUSTRY_BENCHMARKS).map(([key, benchmark]) => (
                      <div key={key} className={`border rounded-lg p-4 ${key === selectedIndustry ? 'border-blue-500 bg-blue-50' : ''}`}>
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-semibold">{benchmark.name}</h4>
                          {key === selectedIndustry && <Badge>Current</Badge>}
                        </div>
                        
                        <div className="grid grid-cols-3 gap-2 text-sm">
                          <div className="text-center">
                            <p className="font-medium">{benchmark.maturityLevel}%</p>
                            <p className="text-xs text-muted-foreground">Security</p>
                          </div>
                          <div className="text-center">
                            <p className="font-medium">{benchmark.nacAdoption}%</p>
                            <p className="text-xs text-muted-foreground">NAC Adoption</p>
                          </div>
                          <div className="text-center">
                            <p className="font-medium">${(benchmark.avgBreachCost / 1000000).toFixed(1)}M</p>
                            <p className="text-xs text-muted-foreground">Avg Breach</p>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Security Maturity Levels</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={Object.entries(INDUSTRY_BENCHMARKS).map(([key, benchmark]) => ({
                        industry: benchmark.name.split(' ')[0],
                        maturity: benchmark.maturityLevel,
                        automation: benchmark.automationLevel,
                        compliance: benchmark.complianceRequirement,
                        current: key === selectedIndustry,
                      }))}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="industry" />
                        <YAxis />
                        <Tooltip />
                        <Bar dataKey="maturity" fill="#3b82f6" name="Security Maturity" />
                        <Bar dataKey="automation" fill="#22c55e" name="Automation Level" />
                        <Bar dataKey="compliance" fill="#eab308" name="Compliance Score" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-center p-2">Security Spend %</th>
                      <th className="text-center p-2">Breach Cost</th>
                      <th className="text-center p-2">Breach Freq</th>
                      <th className="text-center p-2">NAC Adoption</th>
                      <th className="text-center p-2">Maturity</th>
                      <th className="text-center p-2">Automation</th>
                      <th className="text-center p-2">Response Time</th>
                    </tr>
                  </thead>
                  <tbody>
                    {Object.entries(INDUSTRY_BENCHMARKS).map(([key, benchmark]) => (
                      <tr key={key} className={`border-b ${key === selectedIndustry ? 'bg-blue-50' : ''}`}>
                        <td className="p-2 font-medium">{benchmark.name}</td>
                        <td className="text-center p-2">{benchmark.avgSecuritySpend}%</td>
                        <td className="text-center p-2">${(benchmark.avgBreachCost / 1000000).toFixed(1)}M</td>
                        <td className="text-center p-2">{(benchmark.breachFrequency * 100).toFixed(0)}%</td>
                        <td className="text-center p-2">{benchmark.nacAdoption}%</td>
                        <td className="text-center p-2">{benchmark.maturityLevel}%</td>
                        <td className="text-center p-2">{benchmark.automationLevel}%</td>
                        <td className="text-center p-2">{benchmark.incidentResponseTime}h</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="border-l-4 border-blue-500 pl-4">
                    <h4 className="font-semibold text-blue-700">Immediate Actions (0-3 months)</h4>
                    <ul className="mt-2 space-y-1 text-sm">
                      <li>• Conduct comprehensive device inventory and risk assessment</li>
                      <li>•\
